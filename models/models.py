...
class LogisticRegression(L.LightningModule):
    def __init__(self, lr: float = 1e-3, lookback: int = 7):
        super().__init__()
        self.lr = lr
...
class NeuralNet(L.LightningModule):
    def __init__(self, num_features=11, lr=1e-3):
        super().__init__()
        self.lr = lr
    ...
    def validation_step(self, batch, batch_idx):
        x, y = batch
        y_hat = self(x).squeeze()
        val_loss = self.bce_loss(y_hat, y)
    ...
    def test_step(self, batch, batch_idx):
        x, y = batch
        y_hat = self(x).squeeze()
        test_loss = self.bce_loss(y_hat, y)
...
class Kalshi(L.LightningDataModule):
    def __init__(self, batch_size: int = 16):
        super().__init__()
        self.batch_size = batch_size
    
    def setup(self, stage : str):
        dataset = dataloader.KalshiDataset('data/kalshi.csv')
...
class ETH(L.LightningDataModule):
    def __init__(self, batch_size: int = 16, token: str = 'ETHUSD', start_date: str = '2016-11-7', end_date: str = '2024-11-7', lookback: int = 7):
        super().__init__()
        self.batch_size = batch_size
        self.token = token
